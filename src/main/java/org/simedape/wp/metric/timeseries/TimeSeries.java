...
package org.simedape.wp.metric.timeseries;

import org.simedape.wp.metric.util.Arrays;
...
public class TimeSeries {
   ...
   public TimeSeries(double[] timeSeriesArray) {
      ...
      for (int x = 0; x < timeSeriesArray.length; x++) {
         timeReadings.add(timeSeriesArray[x]);
         ArrayList<Double> measurements = new ArrayList<>();
         measurements.add(timeSeriesArray[x]);
         TimeSeriesPoint point = new TimeSeriesPoint(measurements);
         tsArray.add(point);
      }
   }
   ...
   private static boolean determineIsFirstColTime(String filePath)
   {
      ...
      try
      {
         ...
         while ((possibleTimeValues.size()<NUM_OF_VALUES_TO_CMP) && ((line=in.readLine())!=null))
            possibleTimeValues.add(Double.valueOf(extractFirstNumber(line)));
